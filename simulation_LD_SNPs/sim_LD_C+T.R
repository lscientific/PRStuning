...
thresholds <- c(1, 5e-1, 5e-2, 5e-3, 5e-4, 5e-5, 5e-6)
char_thr <- c('1.0000e+00', '5.0000e-01', '5.0000e-02', '5.0000e-03', '5.0000e-04', '5.0000e-05', '5.0000e-06')

res_CT_AUC <- array(NA, dim = c(5, n_rep, 3, 7), dimnames = list(size=2000 * 1:5, data=1:n_rep, Methods = c("Testing", "Unadj. tuning", "PRStuning"), thresholds = thresholds))
...
for(t in 1:length(thresholds)){
  for(n_size in 2:5){
    ...
    for(n_data in 1:n_rep){
      ...
      # get the C+T PRS coefficients
      system(paste0("ldpred p+t --cf=", path, str, "/coord_", str, " --ldr=5 --r2 0.001 ", "--out=", path, str, "/weight --p ", thresholds[t]))
      CT_res <- data.frame(fread(paste0(path, str, '/weight_P+T_r0.00_p', char_thr[t], '.txt')))
      coef <- rep(0, M) 
      coef[CT_res$pos]<- - CT_res$updated_beta # signs are different
      ...
      res_CT_AUC[n_size, n_data, "Testing", t] <- mean(auc_test)
      ...
      res_CT_AUC[n_size, n_data, "Unadj. tuning", t] <- pnorm(delta_hat)
      
      ### PRStuning
      wp <- matrix(NA, N_sample, 10)
      for(i in 1:10){
        R_Beta <- apply(Beta[, ((i-1)*Np+1):(Np*i)], 1, 
                        function(x) as.vector(R %*% diag(1/beta_SE[((i-1)*Np+1):(Np*i)]) %*% matrix(x, nr = Np)) ) # R * Beta[[i]] in 1000*500
        wp[, i] <- apply(R_Beta, 2, function(x) {
          sum(coef[((i-1)*Np+1):(Np*i)] * se_hat[((i-1)*Np+1):(Np*i)] * as.vector(x) / sqrt(Ne_train))
        })
      }
      delta_ad <- rep(0, N_sample) # N_sample of delta_adjusted
      for(i in 1:N_sample){
        delta_ad[i] <- 2 * sum(wp[i, ]) / sqrt(2 * sig2)
      }
      res_CT_AUC[n_size, n_data, "PRStuning", t] <- mean(pnorm(delta_ad)) # posterior mean 0.6801581
      
      cat("\n", "C+T: n_size =", n_size, "n_data =", n_data, "threshold =", thresholds[t], "\n",
          "PRStuning=", round(res_CT_AUC[n_size, n_data, "PRStuning", t], 3),
          "testing=", round(res_CT_AUC[n_size, n_data, "Testing", t], 3), "\n")
    }
  }
  
  save(res_CT_AUC, file = "./LD_sim_C+T_res.RData")
}
...
load("./LD_sim_C+T_res.RData")
thr_chars <- c("1", "5e-1", "5e-2", "5e-3", "5e-4", "5e-5", "5e-6")

dimnames(res_PT_AUC)$size <- 2000 * 1:5
names(dimnames(res_PT_AUC))[3] <- 'Methods'
dimnames(res_PT_AUC)$Methods[c(1, 3, 4)] <- c("Testing", "Unadj. tuning", "PRStuning")
...
p <- list()
for(t in 1:7){
  dat <- as.data.frame.table(res_PT_AUC[2:5, , c(3, 1, 2), t])
  pd <- position_dodge(.6)
...
p_list <- ggarrange(p[[1]], p[[2]], p[[3]], p[[4]], p[[5]], p[[6]], p[[7]], 
                    ncol=2, nrow=4, common.legend = TRUE, legend="right")
p_list
ggsave("./sim_LD_C+T_res.pdf", p_list, width = 4, height = 7)
